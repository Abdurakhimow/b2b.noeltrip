...
## 12. Текущее состояние репозитория
- Добавлен базовый каркас Next.js (App Router, TypeScript) для B2B-кабинета с TailwindCSS и статическими экранами `Home`, `Login`, `Dashboard`.
- Пока нет бизнес-логики, аутентификации, работы с API или БД — это чистый фронтовый scaffold для дальнейшей интеграции.

### Как запустить локально
1. Установить зависимости: `npm install`.
2. Запустить дев-сервер: `npm run dev` (по умолчанию на http://localhost:3000).
3. Проверить линтер: `npm run lint`.

### Следующие логичные шаги
- Подключить аутентификацию (JWT/NextAuth) и защиту маршрутов `/dashboard` и будущих экранов.
- Добавить Prisma + PostgreSQL, описать модели `Agency`, `AgencyUser`, `AdminUser`, `Booking` и подготовить миграции.
- Реализовать формы регистрации/логина с валидацией, списки заявок и карточки с вкладками.
- Интегрировать загрузку документов через Hetzner Object Storage с pre-signed URL.
